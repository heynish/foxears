...
export async function overlayImages(baseImagePath: string, overlayImagePath: string, outputFileName: string, options: OverlayOptions = {}): Promise<ImageDetails> {
  ...
  try {
    ...
    console.time('Image Manipulation Time');
    ...
    const x = options.x ?? (baseImage.bitmap.width / 2) - (diameter / 2);
    const y = options.y ?? (baseImage.bitmap.height / 2) - (diameter / 2);
    ...
    console.timeEnd('Image Manipulation Time');

    console.time('S3 Upload Base Image Time');
    // Upload base image
    ...
    console.timeEnd('S3 Upload Base Image Time');

    console.time('Overlay Positioning and S3 Upload Time');
    ...
    console.timeEnd('Overlay Positioning and S3 Upload Time');
